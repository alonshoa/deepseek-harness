...
Use `ask_advisor` when one of these conditions holds:

- two materially different attempts to solve the same failure have failed;
- the root cause is unclear and another local attempt would mostly be guessing;
...
Do not use the advisor for routine edits, formatting, straightforward test failures, or work you can confidently complete locally.

## Delegation contract

When calling `ask_advisor`, always set `run_in_background` to false if that argument is available. The worker must wait for the advice before continuing.
...
If the recommended change fails, record the new evidence before asking again. A second escalation must explain why the previous recommendation did not resolve the problem.
